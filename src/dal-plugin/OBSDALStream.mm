...
- (CFTypeRef)clock {
    if (_clock == NULL) {
        OSStatus err = CMIOStreamClockCreate(kCFAllocatorDefault, CFSTR("obs-mac-virtualcam::OBSDALStream::clock"), (__bridge void *)self,  CMTimeMake(1, 10), 100, 10, &_clock);
        if (err != noErr) {
...
- (NSImage *)testCardImage {
    if (_testCardImage == nil) {
        _testCardImage = OBSDALImageOfTestCardWithSize(self.testCardSize);
    }
...
- (CVPixelBufferRef)createPixelBufferWithTestAnimation {
    ...
    OBSDALDrawDialWithFrame(NSMakeRect(0, 0, width, height), (int(self.fps) - self.sequenceNumber % int(self.fps)) * 360 / int(self.fps));
...
- (void)fillFrame {
    ...
    CMSampleTimingInfo timingInfo = OBSDALCMSampleTimingInfoForTimestamp(hostTime, self.fps, 1);
...
- (void)queueFrameWithSize:(NSSize)size timestamp:(uint64_t)timestamp fpsNumerator:(uint32_t)fpsNumerator fpsDenominator:(uint32_t)fpsDenominator frameData:(NSData *)frameData {
    ...
    CMSampleTimingInfo timingInfo = OBSDALCMSampleTimingInfoForTimestamp(timestamp, fpsNumerator, fpsDenominator);
    ...
    OBSDALCMSampleBufferCreateFromData(size, timingInfo, self.sequenceNumber, frameData, &sampleBuffer);
...
- (UInt32)getPropertyDataSizeWithAddress:(CMIOObjectPropertyAddress)address qualifierDataSize:(UInt32)qualifierDataSize qualifierData:(nonnull const void *)qualifierData {
    switch (address.mSelector) {
        ...
        default:
            DLog(@"OBSDALStream unhandled getPropertyDataSizeWithAddress for %@", [OBSDALObjectStore StringFromPropertySelector:address.mSelector]);
            return 0;
    };
}
    ...
    switch (address.mSelector) {
        ...
        case kCMIOStreamPropertyClock:
            *static_cast<CFTypeRef*>(data) = self.clock;
            ...
            // code does.
            // https://github.com/lvsti/CoreMediaIO-DAL-Example/blob/0392cb/Sources/Extras/CoreMediaIO/OBSDALDeviceAbstractionLayer/OBSDALDevices/DP/Properties/CMIO_DP_Property_Clock.cpp#L75
            CFRetain(*static_cast<CFTypeRef*>(data));
            *dataUsed = sizeof(CFTypeRef);
            break;
        default:
            DLog(@"OBSDALStream unhandled getPropertyDataWithAddress for %@", [OBSDALObjectStore StringFromPropertySelector:address.mSelector]);
            *dataUsed = 0;
    };
}
...
- (BOOL)hasPropertyWithAddress:(CMIOObjectPropertyAddress)address {
    switch (address.mSelector){
        ...
        case kCMIOStreamPropertyOutputBuffersNeededForThrottledPlayback:
            ...
            return false;
        default:
            DLog(@"OBSDALStream unhandled hasPropertyWithAddress for %@", [OBSDALObjectStore StringFromPropertySelector:address.mSelector]);
            return false;
    };
}

- (BOOL)isPropertySettableWithAddress:(CMIOObjectPropertyAddress)address {
    DLog(@"OBSDALStream unhandled isPropertySettableWithAddress for %@", [OBSDALObjectStore StringFromPropertySelector:address.mSelector]);
    return false;
}
...
- (void)setPropertyDataWithAddress:(CMIOObjectPropertyAddress)address qualifierDataSize:(UInt32)qualifierDataSize qualifierData:(nonnull const void *)qualifierData dataSize:(UInt32)dataSize data:(nonnull const void *)data {
    DLog(@"OBSDALStream unhandled setPropertyDataWithAddress for %@", [OBSDALObjectStore StringFromPropertySelector:address.mSelector]);
}
